...
async function findProductImages(productId) {
    ...
    try {
        ...
        const productImagesArray = productImagesQuerySnapshot.docs.map(doc => doc.data());
        ...
        const allImages = imagesQuerySnapshot.docs.map(doc => doc.data().publicUrl).filter(Boolean);

        return allImages;
    } catch (error) {
        console.error(`Error fetching product images for productId: ${productId}`, error);
        return [];
    }
}
    ...
    try {
        async function fetchProductsByType(productType) {
            try {
                ...
                const querySnapshot = await productsRef.where('productType', '==', productType).get();
        ...
        const [plants, zenpots, groboxes, accessories] = await Promise.all([
