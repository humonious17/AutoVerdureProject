...
const CollectionCard = ({ title, description, image, video }) => {
  ...
  return (
    <div
      className="w-full h-auto md:w-full md:h-auto p-3 md:p-0 flex flex-col md:flex-row-reverse md:rounded-[32.1px] xl:rounded-[56px] md:overflow-hidden bg-[#F8F8F8] border-[2.86px] border-[#fffbf7] hover:border-primaryMain rounded-[10px] drop-shadow-xl transition-all duration-500 ease-in-out"
    ...
    >
      ...
      >
        {isVideo ? (
          <video
            className="object-cover w-[203.231px] h-[170px] md:w-[203.231px] md:h-full xl:w-[355px] transition-transform duration-500 ease-in-out"
            src={video}
            autoPlay
            loop
            muted
