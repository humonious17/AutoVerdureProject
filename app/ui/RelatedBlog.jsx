...
const RelatedBlog = ({ title, description }) => {
  ...
  const scrollLeft = () => {
    ...
    if (containerRef.current) {
      containerRef.current.scrollBy((currentIndex === 1)
        ? { left: -(300.35 + ((containerRef.current.offsetWidth - 900) / 4)), behavior: 'smooth' }
        : { left: -(300 + ((containerRef.current.offsetWidth - 900) / 4)), behavior: 'smooth' });
    }
  };
  ...
  const scrollRight = () => {
    setCurrentIndex(currentIndex + 1);
    if (containerRef.current) {
      containerRef.current.scrollBy(((currentIndex === 0)
        ? { left: (300.35 + ((containerRef.current.offsetWidth - 900) / 4)), behavior: 'smooth' }
        : { left: (300 + ((containerRef.current.offsetWidth - 900) / 4)), behavior: 'smooth' }));
    }
  };
