...
async function findProductImages(productId) {
  ...
  if (!db) {
    console.warn("Firestore db is not initialized. Returning empty array for product images.");
    return [];
  }
...
export default async function updateProductsCache() {
  try {
    async function fetchProductsByType(productType) {
      try {
        if (!db) {
          console.warn("Firestore db is not initialized. Returning empty array for products.");
          return [];
        }
    ...
    const [plants, accessories, planters, flowers] = await Promise.all([
